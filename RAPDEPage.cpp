...
BOOL RAPDEPage::WritePDE()
{
	...
	TRY
	{
		...
		// check if m_sOutputDirectory has a trailing backslash, if it does, remove it
		//i = (int)strlen(m_sOutputDirectory);
		//if ( m_sOutputDirectory[i-1] == '\\' )
		//{
		//	m_sOutputDirectory[i-1] = '\0';
		//}
		sFile.Format(_T("%s\\%s\\"), m_sOutputDirectory, sFilename);
...
// Autogenerated file by RAGenPDE (v%s, (%s)\r\n\
// %s.pde\r\n\
//\r\n\
