...
// %s.pde\r\n\
//\r\n\
// This version designed for Standard Libraries v0.8.3\r\n%s"), 
					cb_GetFileVersionString(AfxGetInstanceHandle()),
					t.Format(_T("%m/%d/%Y %H:%M")),
					sFilename,
					(fBanner)?"// WebBanner features require v0.8.3.9 or later\r\n":"");
...
{\r\n\
    ...
    ReefAngel.LoadMenu(pgm_read_word(&(menu_items[0])),SIZE(menu_items)); // Load menu items\r\n\
");
		f.Write(s, s.GetLength());
    ...
    ReefAngel.Timer[4].Interval=%d;  // set interval to %d seconds\r\n\
